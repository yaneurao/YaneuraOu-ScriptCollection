...
`dlshogi-trainer.py` は、`C:\shogi\DeepLearningShogi` の `dlshogi.train` または `dlshogi.ptl` を、教師ファイル1個ずつ呼び出して学習を進めるためのラッパーです。
...
```powershell
python .\dlshogi-trainer.py --backend ptl
```
...
```powershell
python .\dlshogi-trainer.py --start_index 5
```
...
ログ抽出も `dlshogi-trainer.py` だけでできます。

```powershell
python .\dlshogi-trainer.py --network exp___i20x256 --show_log
```
...
```powershell
python .\dlshogi-trainer.py --out_dir C:\shogi\model\exp___i20x256_round2 --show_log
```
...
```powershell
python .\dlshogi-trainer.py --backend ptl --use_compile
```
...
```powershell
python .\dlshogi-trainer.py --backend ptl --use_compile --compile_backend inductor
```
...
```txt
C:\shogi\model\<network>\_torchinductor_cache\train-XXXX-<pid>
```

このフォルダはコンパイルキャッシュなので、学習が止まっているときなら削除しても構いません。
...
これは行列積を Tensor Core に合わせてパディングする最適化を切る設定です。速度面では少し損をする可能性がありますが、Windowsでまず安定して `inductor` を通すための回避策です。
...
```powershell
python .\dlshogi-trainer.py --backend ptl --use_compile --compile_backend inductor --compile_fullgraph
```
...
```powershell
python .\dlshogi-trainer.py --backend ptl --use_compile --compile_backend inductor --compile_mode reduce-overhead
```
...
以下は `C:\shogi\DeepLearningShogi` を git clone し、`C:\shogi\learner` にこのラッパーを置いて使う前提です。
...
`dubious ownership` などのエラーで git が止まる場合は、そのマシンで以下を一度だけ実行します。
