...
def snapshot_book_position_for_write(book:Book, sfen:Sfen)->tuple[str, list[MoveInfo]]:
    with book.lock:
    ...
    return f"{sfen} {ply}", moveinfos


def write_ybb_book_records(book:Book, path:str, ply_limit:int|None, sfens:list[Sfen]|None = None)->int:
    if sfens is None:
    ...
    try:
        book_for_write : dict[str, list[BookLib.BookMove]] = {}
        for count, sfen in enumerate(sfens, 1):
            sfen_with_ply, moveinfos = snapshot_book_position_for_write(book, sfen)
            book_for_write[sfen_with_ply] = [
                BookLib.BookMove(move_info.move, "none", move_info.eval, move_info.depth, 1) # type:ignore[arg-type]
        ...
        BookLib.write_yaneuraou_book(book_for_write, temp_path)
...
def write_yaneuraou_book_records(book:Book, path:str, ply_limit:int|None, sfens:list[Sfen]|None = None)->int:
    if BookLib.is_ybb_path(path):
        return write_ybb_book_records(book, path, ply_limit, sfens)
...
def save_book_backup(book:Book, save_dir:str, ply_limit:int|None = None)->str:
    ...
    path = os.path.join(
        save_dir,
        f"{BOOK_DB_NAME}-{make_time_stamp()}_{len(sfens)}{ply_suffix}{BOOK_BACKUP_EXTENSION}"
    )
...
def parse_position_command_entry(line:PositionStr)->PositionCommandEntry:
    """
    think_sfens.txt の1行を、局面コマンド本体とメタ情報に分解する。
    ...
    for raw_meta in parts[1:]:
        ...
        if key == "book_extend_ply":
            if is_none_argument(value):
                book_extend_ply = None
            else:
                book_extend_ply = int(value)
                if book_extend_ply < 0:
                    raise ValueError("book_extend_ply must be non-negative integer or None")
        elif key == "eval_limit":
            if is_none_argument(value):
                eval_limit = None
            else:
                eval_limit = int(value)
                if eval_limit < 0:
                    raise ValueError("eval_limit must be non-negative integer or None")
        elif key == "game_ply_limit":
            if is_none_argument(value):
                max_book_ply = None
            else:
                max_book_ply = int(value)
                if max_book_ply <= 0:
                    raise ValueError("game_ply_limit must be positive integer or None")

    return PositionCommandEntry(position_cmd, book_extend_ply, eval_limit, max_book_ply)


def format_position_command_entry(entry:PositionCommandEntry)->str:
    metadata : list[str] = []
    if entry.book_extend_ply is not None:
        metadata.append(f"book_extend_ply={entry.book_extend_ply}")
    if entry.eval_limit is not None:
        metadata.append(f"eval_limit={entry.eval_limit}")
    if entry.max_book_ply is not None:
        metadata.append(f"game_ply_limit={entry.max_book_ply}")
    if not metadata:
        return entry.position_cmd
    return f"{entry.position_cmd}, " + ", ".join(metadata)


def book_extend_ply_rank(book_extend_ply:int|None)->int:
    return -1 if book_extend_ply is None else book_extend_ply
...
def merge_position_command_entries(old:PositionCommandEntry|None, new:PositionCommandEntry)->PositionCommandEntry:
    if old is None:
        return new
    return PositionCommandEntry(
        new.position_cmd,
        max(old.book_extend_ply, new.book_extend_ply, key=optional_int_rank),
        max(old.eval_limit, new.eval_limit, key=optional_int_rank),
        max(old.max_book_ply, new.max_book_ply, key=optional_int_rank),
    )


def add_position_command_entry(
    entries:dict[PositionStr, PositionCommandEntry],
    position_cmd:PositionStr,
    book_extend_ply:int|None = None,
    eval_limit:int|None = None,
    max_book_ply:int|None = None,
):
    new_entry = PositionCommandEntry(position_cmd, book_extend_ply, eval_limit, max_book_ply)
    entries[position_cmd] = merge_position_command_entries(entries.get(position_cmd), new_entry)


def write_position_command_entries(path:str, entries:dict[PositionStr, PositionCommandEntry]):
    with open(path, 'w') as w:
        for entry in entries.values():
            w.write(format_position_command_entry(entry) + '\n')
...
def estimate_book_position_count_from_name(path:str)->int|None:
    """BookMiner backup / peta book のファイル名末尾の _N.db / _N.ybb から局面数を読む。"""
    filename = os.path.basename(path)
    for prefix in (BOOK_DB_NAME, PETA_BOOK_DB_NAME):
        pattern = rf"^{re.escape(prefix)}-\d{{14}}_(\d+)(?:\.db|\.ybb)$"
...
def read_yaneuraou_book_file(
    ...
    *,
    ...
    """
    if BookLib.is_ybb_path(path):
        return read_ybb_book_file(
            path,
            on_position,
            normalize_eval=normalize_eval,
            copy_moveinfos=copy_moveinfos,
        )
...
def read_ybb_book_file(
    ...
    *,
    ...
    """
    CommonLib の .ybb reader から BookMiner 内部形式へ変換して読み込む。
    .db は旧フォーマット互換のため BookMiner 側の既存パーサを使う。
    """
    total_positions, _flags = BookLib.read_ybb_header(Path(path))
    count = 0
    print_book_read_start(path, total_positions)

    for sfen_with_ply, book_moves in BookLib.read_yaneuraou_book_blocks(path):
        count += 1
        if count % BOOK_READ_PROGRESS_INTERVAL == 0:
            print_book_read_progress(count, total_positions)
        ...
        moveinfos = [
            MoveInfo(
                book_move.move,
                normalize_book_eval(book_move.value) if normalize_eval else book_move.value,
                book_move.depth,
            )
        ...
        on_position(
            ...
            [
                MoveInfo(moveinfo.move, moveinfo.eval, moveinfo.depth)
                for moveinfo in moveinfos
            ] if copy_moveinfos else moveinfos,
        )
...
def collect_book_backup_paths()->list[str]:
    ...
    for filename in os.listdir(BOOK_BACKUP_DIR):
        if not filename.startswith(f"{BOOK_DB_NAME}-"):
            continue
        if not is_supported_peta_book_path(filename):
            continue
        if "_ply" in filename:
            continue
...
def get_latest_book_backup_or_none()->str|None:
    ...
    ply制限つきの `*_plyN` は、部分書き出しなので自動選択から除外する。
...
def get_latest_book_backup()->str:
    ...
    if path is None:
        raise Exception(f"book backup file not found : {BOOK_BACKUP_DIR}/{BOOK_DB_NAME}-*.db or *.ybb")
    return path
...
def get_latest_peta_book()->str:
    ...
    if not paths:
        raise Exception(f"peta book file not found : {BOOK_BACKUP_DIR}/{PETA_BOOK_DB_NAME}-*.db or *.ybb")
    return paths[-1]


def is_supported_peta_book_path(path:str)->bool:
    return os.path.splitext(path)[1].lower() in (".db", ".ybb")


def ensure_supported_peta_book_path(path:str, label:str):
    if not is_supported_peta_book_path(path):
        raise Exception(f"{label} must be .db or .ybb : {path}")
...
def parse_regular_book_backup_name(path:str)->tuple[str,int]|None:
    """
    book_miner-YYYYMMDDHHMMSS_N.db / .ybb から timestamp と局面数を取り出す。
    peta book はこの2つを引き継いだ名前にする。
    """
    filename = os.path.basename(path)
    pattern = rf"^{re.escape(BOOK_DB_NAME)}-(\d{{14}})_(\d+)(?:\.db|\.ybb)$"
    match = re.fullmatch(pattern, filename)
    if match is None:
        return None
                        ...
                        if refutation_book is not None:
                            ...
                            if next_position_info is None:
                                if (
                                    ...
                                    add_position_command_entry(think_sfens, next_position_cmd, book_extend_ply, eval_limit, max_book_ply)
                                continue
...
def peta_leaf_rejoins_peta_book(
    sfen_with_ply:Sfen,
    leaf_eval:int|None = None,
    eval_drop:int|None = None,
)->bool:
    global peta_book

    filter_by_eval_drop = (
        eval_drop is not None and eval_drop > PETA_REJOIN_EVAL_DROP_DISABLED
    )
    board = cshogi.Board(sfen_with_ply)
    for move32 in list(board.legal_moves):
        ...
        position_info, flipped_bookhit = find_book_position_with_flip(peta_book, next_sfen)
        if position_info is None:
            continue

        if not filter_by_eval_drop:
            return True

        if leaf_eval is None:
            continue

        best_info, _ = best_moveinfo_in_sfen_orientation(position_info, flipped_bookhit)
        if best_info is None or not isinstance(best_info.eval, int):
            continue

        if leaf_eval - best_info.eval >= eval_drop:
            return True
...
def peta_rejoin(
    ...
    print(
        ...
        f"max_step = {max_step}, max_book_ply = {max_book_ply}, "
        f"eval_drop = {'None' if eval_drop is None else eval_drop}, "
        f"book_extend_ply = {'None' if book_extend_ply is None else book_extend_ply}, "
        f"eval_limit = {'None' if eval_limit is None else eval_limit}, "
        f"start_sfens_path = {start_sfens_path}"
    )
    ...
    for turn in [1, 0]:
        ...
        visited : set[Sfen] = set()
        current_positions : dict[PositionStr, tuple[Sfen,int,int,int|None]] = {}
        ...
        for position_cmd, sfen_with_ply in load_peta_root_positions(start_sfens_path):
            ...
            if position_info is None or not position_info.moveinfos:
                total_leaf_nodes += 1
                if peta_leaf_rejoins_peta_book(sfen_with_ply, None, eval_drop):
                    add_position_command_entry(think_sfens, position_cmd, book_extend_ply, eval_limit, max_book_ply)
            ...
            current_positions[position_cmd] = (sfen_with_ply, root_best, peta_eval_diff, None)
            print(f"root sfen : {sfen_with_ply} , root_best = {root_best}")
        ...
        while current_positions:
            ...
            next_positions : dict[PositionStr, tuple[Sfen, int, int, int|None]] = {}

            for position_cmd, (sfen_with_ply, root_best_eval, peta_eval_diff0, leaf_eval) in current_positions.items():
                ...
                if position_info is None or not position_info.moveinfos:
                    total_leaf_nodes += 1
                    if peta_leaf_rejoins_peta_book(sfen_with_ply, leaf_eval, eval_drop):
                        add_position_command_entry(think_sfens, position_cmd, book_extend_ply, eval_limit, max_book_ply)
                        total_rejoin_nodes += 1
                    continue
                ...
                for moveinfo in moveinfos:
                    ...
                    if eval_low <= moveinfo.eval:
                        ...
                        next_position_cmd = append_position_move(position_cmd, move)
                        next_positions[next_position_cmd] = (next_sfen, - root_best_eval, peta_eval_diff0, moveinfo.eval)
...
def collect_opponent_book_paths()->list[str]:
    ...
    paths : list[str] = []
    for filename in os.listdir(BOOK_OPPONENT_DIR):
        path = os.path.join(BOOK_OPPONENT_DIR, filename)
        if os.path.isfile(path) and is_supported_peta_book_path(path):
            paths.append(path)
    paths.sort(key=lambda path: os.path.basename(path))
    return paths
...
def candidate_best_moves_from_moveinfos(moveinfos:list[MoveInfo], flipped_bookhit:bool, eval_diff:int)->list[tuple[MoveStr, int]]:
    ...
    for moveinfo in moveinfos:
        ...
        if moveinfo.eval < eval_low:
            continue
        move = flipped_move(moveinfo.move) if flipped_bookhit else moveinfo.move
        moves.append((move, moveinfo.eval))
    return moves
...
def peta_opponent(
    eval_diff:int = PETA_OPPONENT_DEFAULT_EVAL_DIFF,
    max_book_ply:int = MAX_BOOK_PLY,
    max_step:int = PETA_DEFAULT_MAX_STEP,
    book_extend_ply:int|None = None,
    eval_limit:int|None = None,
    ...
    """
    book/book_opponent/ に置いた過去配布bookと、現在読み込んでいるpeta_bookを仮想対局させる。
    双方が自分の手番でbest候補を指し、どちらかのbookが切れた地点から現在peta_bookのPV leafまで辿って
    book/think_sfens.txt に書き出す。
    """

    print(
        f"peta_opponent, eval_diff = {eval_diff}, max_book_ply = {max_book_ply}, "
        f"max_step = {max_step}, book_extend_ply = {'None' if book_extend_ply is None else book_extend_ply}, "
        f"eval_limit = {'None' if eval_limit is None else eval_limit}, "
        f"opponent_dir = {BOOK_OPPONENT_DIR}"
    )

    opponent_paths = collect_opponent_book_paths()
...
def put_position_commands(
    ...
    for line_number, line in raw_lines:
        try:
            entry = parse_position_command_entry(line)
            decode_position_string(entry.position_cmd)
        except Exception as exc:
            skipped += 1
    ...
    effective_eval_limit_values = {
        eval_limit if entry.eval_limit is None else entry.eval_limit
        for entry in entries.values()
    }
    if len(effective_eval_limit_values) <= 1:
        job_eval_limit : int | str = next(iter(effective_eval_limit_values), eval_limit)
    else:
    ...
    for entry in entries.values():
        task_eval_limit = eval_limit if entry.eval_limit is None else entry.eval_limit
        task_max_book_ply = max_book_ply if entry.max_book_ply is None else entry.max_book_ply
        task_book_extend_ply = book_extend_ply if entry.book_extend_ply is None else entry.book_extend_ply
    ...
    while True:
        try:
            ...
            if i == 'h':
                ...
                print("  PJ   : peta rejoin , pj (eval_drop) (peta_eval_diff) (max_step) (game_ply_limit) (book_extend_ply) (eval_limit)")
                ...
                # peta_rejoin
                has_eval_drop = len(inp) >= 7
                eval_drop = (
                    parse_int_argument(inp, 1, PETA_REJOIN_EVAL_DROP_DISABLED)
                    if has_eval_drop
                    else None
                )
                base_index = 2 if has_eval_drop else 1
                peta_eval_diff = parse_int_argument(inp, base_index, command_defaults.eval_diff)
                max_step = parse_int_argument(inp, base_index + 1, command_defaults.max_step)
                max_book_ply = parse_int_argument(inp, base_index + 2, command_defaults.game_ply_limit)
                book_extend_ply = parse_int_argument(inp, base_index + 3, command_defaults.book_extend_ply)
                eval_limit = parse_int_argument(inp, base_index + 4, command_defaults.eval_limit)
                if peta_eval_diff < 0:
                    print("Error : peta_eval_diff must be non-negative integer.")
                    continue
                if max_book_ply <= 0:
