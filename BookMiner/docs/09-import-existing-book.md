...
一方、`enqueue` の `eval_limit` は、`book/think_sfens.txt` の各行で到達した定跡木 leaf から、どの指し手を延長するかを決める値です。

`enqueue` は `book/think_sfens.txt` の各行を先頭局面から順に再生しますが、途中の非 leaf 局面の評価値では打ち切りません。
例えば平手開始局面の評価値が `800` で、`eval_limit 400` であっても、その局面は単に通過します。

ただし、到達した定跡木 leaf から出る指し手の評価値が `eval_limit` を超えている場合、その指し手の先は延長しません。
