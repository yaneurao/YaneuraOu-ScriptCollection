...
この章では、BookMiner の運用で出てくる `peta_shock`、`peta_read`、`peta_next`、`peta_refutation`、`peta_depth_gap` が何をしているのか、なぜ必要なのかを説明します。
...
2. peta_book を peta_next、peta_refutation、peta_depth_gap のいずれかで辿り、次に掘る局面を book/think_sfens.txt に書き出す
...
一方で、peta shock 化によって、もともと2番手以下だった指し手が best に入れ替わることがあります。BookMiner ではこれを「反駁」と呼びます。反駁された指し手が depth 0 のままだと、その評価値はまだ十分に延長されていない可能性があり、root 側へ強いノイズとして戻ることがあります。この候補を重点的に延長するために `peta_refutation` を使います。また、best に近い評価値だが depth が浅い候補手を延長するために `peta_depth_gap` を使います。
...
| `book/backup/peta_book-....db` | 通常定跡 DB を peta shock 化した DB。`peta_next`、`peta_refutation`、`peta_depth_gap`、対局用の基本入力です。 |
...
`peta_next` の `eval_diff`、`peta_refutation` の `eval_refutation_margin`、`peta_depth_gap` の `eval_per_ply`、`enqueue` の `eval_limit` は別の値です。
...
- `peta_next`、`peta_refutation`、`peta_depth_gap` はメモリ上に読み込まれている peta_book を辿ります。DB ファイルを毎回読み直すわけではありません。
