...
この章では、BookMiner の運用で出てくる `peta_shock`、`peta_read`、`peta_next`、`peta_next_refutation`、`peta_refutation`、`peta_depth_gap` が何をしているのか、なぜ必要なのかを説明します。
...
2. peta_book を peta_next、peta_next_refutation、peta_refutation、peta_depth_gap のいずれかで辿り、次に掘る局面を book/think_sfens.txt に書き出す
...
| `book/backup/peta_book-....db` | 通常定跡 DB を peta shock 化した DB。`peta_next`、`peta_next_refutation`、`peta_refutation`、`peta_depth_gap`、対局用の基本入力です。 |
...
| `peta next refu.` | `nf eval_diff [max_step] [max_book_ply] [eval_refutation_margin]` | `peta_next` の leaf のうち、元DBでは best ではなかった反駁leafだけを `book/think_sfens.txt` に書き出します。 |
| `peta refutation` | `f eval_refutation_margin [eval_limit]` | 反駁された depth 0 best のうち、peta shock後の旧best手との差が大きい候補を `book/think_sfens.txt` に書き出します。`eval_limit` 指定時は enqueue 前に retire が確定している候補を除外します。 |
...
通常は `peta_shock` → `peta_next` → `enqueue` を繰り返します。通常の leaf 延長のうち反駁されたものだけを優先したい場合は `peta next refu.`、反駁された depth 0 best を重点的に延長したい場合は `peta refutation`、best に近いが浅すぎる候補を延長したい場合は `peta depth_gap` を使います。メモリや時間の都合で別マシンで peta shock 化する場合は、外部で作った `peta_book-....db` を `book/backup/` に置き、`peta_read` → `peta_next` / `peta next refu.` / `peta refutation` / `peta depth_gap` → `enqueue` と進めます。
...
例えば peta shock 前に旧bestが `50`、反駁候補手が `49` でも、peta shock 後に旧best手が `-300`、反駁候補手が `49 depth 0` なら差は `349` です。`eval_refutation_margin` が `349` 以下なら抽出対象になります。`f` コマンドで値を省略した場合のデフォルトは `100` です。
...
GUIでは `peta next refu.`、CLIでは `nf` コマンドです。
...
`peta_next` / `peta_next_refutation` の `eval_diff`、`peta_refutation` / `peta_next_refutation` の `eval_refutation_margin`、`peta_depth_gap` の `eval_per_ply`、`enqueue` の `eval_limit` は別の値です。

| 値 | 使う場所 | 意味 |
|---|---|---|
| `eval_diff` | `peta_next` / `n`、`peta_next_refutation` / `nf` | peta_book の中で、root の best move からどれくらい評価値が離れた枝まで辿るか。 |
| `eval_refutation_margin` | `peta_refutation` / `f`、`peta_next_refutation` / `nf` | peta shock後の反駁候補手と旧best手の評価値差がどれくらい以上なら抽出するか。 |
| `eval_per_ply` | `peta_depth_gap` / `d` | bestとのdepth差1手あたり、候補手の評価値がどれくらい改善しうると仮定するか。 |
...
- `peta_next`、`peta_next_refutation`、`peta_refutation`、`peta_depth_gap` はメモリ上に読み込まれている peta_book を辿ります。DB ファイルを毎回読み直すわけではありません。
