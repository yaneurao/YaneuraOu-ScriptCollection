...
`コマンド受付を開始しました。` と表示されるまでは、`peta_shock`、`peta_next`、`enqueue`、`定跡DBのbackup` などの操作ボタンは無効です。
...
`エンジン起動` は、`settings/engine_settings.json5` の `multi` の合計数に対して、`readyok` を返したエンジン数を表示します。
SSH経由で複数PCを使う場合も、実際に起動して `readyok` を返した数で進みます。
