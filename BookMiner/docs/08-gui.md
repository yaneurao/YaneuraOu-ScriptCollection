...
既存定跡から掘り始める場合、`eval_limit` が小さいと leaf に到達する前に打ち切られ、探索ログが出ず局面数も増えないことがあります。初回延長では `99999` のように十分大きな値を指定してください。
...
`ログをタブ表示` をオンにすると、1つのログ欄をタブで切り替えて広く表示します。
オフにすると、従来通り4つのログ欄を縦に並べて表示します。初回起動時のデフォルトはオフです。
ログ内容は両方の表示に同時に書き込まれるので、表示を切り替えてもログは失われません。
