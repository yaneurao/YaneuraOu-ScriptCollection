...
BookMiner は、やねうら王の標準定跡フォーマット形式でファイルに保存します。
終了時に`book/book_miner.db`に書き出し、次回起動時に、前回保存したファイルから読み込んで再開します。
...
- `book/book_miner.db` : BookMiner が作業用に読み書きする定跡 DB。
- `book/backup/book_miner-....db` : `w` コマンドなどで書き出されるバックアップ。
- `book/peta_book.db` : peta shock 化の結果として作られる定跡 DB。

## 通常定跡 DB

BookMiner が `w` コマンドで書き出す、変換前の定跡 DB です。

ファイル名は次のようになります。

```text
book/backup/book_miner-20260607071000_123.db
```

`20260607071000`の部分は書き出した時刻(タイムスタンプ)で、そのあとの`_123`の部分はply(手数)です。この手数までを書き出したという意味です。ply = すべて まで書き出す場合は、ファイル名に`_123`の部分は付与されません。
...
BookMiner の `book/book_miner.db`、`book/backup/book_miner-....db`、`book/peta_book.db` は、この形式で保存されます。
...
やねうら王V9.40を Windows(x64) AVX2用にビルドしたものを、すでに置いてあります。Windows(x64)以外の環境で動かしたい場合は、御自分でビルドしてください。

ビルド用の make コマンドは、例えば次のように書きます。(TARGET_CPUのところをお使いのPCのCPUに合わせて変更してください。)
...
BookMiner では、すでに掘った定跡ツリーの末端から、次に掘るべき局面を探します。
...
通常は平手初期局面ですが、`book/peta_start_sfens.txt` を使うと開始局面集合を指定できます。
...
BookMiner の `w 100` は、初期局面から 100 手目までの局面を書き出す、という意味です。

## peta_next

peta shock 化した定跡 DB から、次に掘るべき leaf 局面を求める処理です。

BookMiner では `n` コマンドがこの役割を持ちます。
