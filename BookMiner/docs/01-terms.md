...
を満たすものだけを `peta refutation` の抽出対象にします。
...
BookMiner では、次に掘る leaf の列挙や、対局用定跡の作成に使います。

変換そのものは [10. peta shock 化](10-peta-shock.md)、peta book を使って次に掘る局面を作る操作は [11. peta book を使って次に掘る局面を作る](11-peta-operations.md) を参照してください。
...
BookMiner の CLI では `pr eval_refutation_margin [eval_diff] [max_step] [game_ply_limit] [book_extend_ply] [eval_limit]`、GUI では `peta refutation` ボタンに対応します。抽出結果は `book/think_sfens.txt` に書き出されます。詳しくは [11. peta book を使って次に掘る局面を作る](11-peta-operations.md#peta-refutation) を参照してください。
...
`book/think_unsolved_sfens.txt` にある棋譜の各途中局面から、peta_book 上の best PV を leaf まで辿り、次に掘る局面として `book/think_sfens.txt` に書き出す処理です。

BookMiner の CLI では `pu [eval_drop_limit] [max_step] [game_ply_limit] [book_extend_ply] [eval_limit]`、GUI では `peta unsolved` ボタンに対応します。`None` を指定するとデフォルト値を使います。`eval_drop_limit` は棋譜rootからどれだけ悪化した途中局面を除外するかです。書き出し後の enqueue は手動で実行します。詳しくは [11. peta book を使って次に掘る局面を作る](11-peta-operations.md#peta-unsolved) を参照してください。
...
BookMiner の CLI では `po [eval_diff] [max_step] [game_ply_limit] [book_extend_ply] [eval_limit]`、GUI では `peta opponent` ボタンに対応します。`book_extend_ply`、`eval_limit`、`game_ply_limit` を指定すると、書き出し行にメタ情報が付き、その行だけ enqueue 時の探索条件を上書きします。詳しくは [11. peta book を使って次に掘る局面を作る](11-peta-operations.md#peta-opponent) を参照してください。
