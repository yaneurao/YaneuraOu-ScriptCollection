...
BookMiner では、次に掘る leaf を探す `peta_next`、反駁された leaf を探す `peta_refutation`、depth差で延長候補を探す `peta_depth_gap`、負け棋譜周辺を掘る `peta_unsolved`、過去配布定跡への対策候補を掘る `peta_opponent`、対局用定跡の作成に使います。
