...
GUI の `enqueue進捗` は、完了したタスク数をもとに表示されます。BookMiner 起動後に enqueue した累計タスクに対して、どこまで完了したかを確認できます。複数回 enqueue した場合、分母は追加分だけ増えます。進捗ログはおおむね 10 秒ごとに、前回出力時から完了数または `deferred` が変わっている job について出力されます。
