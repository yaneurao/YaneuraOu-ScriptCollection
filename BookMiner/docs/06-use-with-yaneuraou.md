...
```text
BookMiner/book/backup/peta_book-20260607103251_14505901.ybb
```

これは `p` コマンド、または外部の `makebook peta_shock` で peta shock 化された定跡です。

`w` コマンドで作られる `book/backup/book_miner-....ybb` もやねうら王の通常定跡形式ですが、対局用には peta shock 化後の `peta_book-....ybb` を使うのが基本です。
...
やねうら王エンジンの実行ファイルがあるフォルダに `book/` フォルダを用意し、使いたい `peta_book-....ybb` をコピーします。
...
engine_options.txt
book/
  peta_book-20260607103251_14505901.ybb
```
...
```text
BookDir book
BookFile peta_book-20260607103251_14505901.ybb
USI_OwnBook true
FlippedBook true
IgnoreBookPly true
```
...
setoption name BookFile value peta_book-20260607103251_14505901.ybb
...
- `BookFile` が使いたい `peta_book-....ybb` になっているか。
- `BookDir` から見た場所に、その `peta_book-....ybb` があるか。
