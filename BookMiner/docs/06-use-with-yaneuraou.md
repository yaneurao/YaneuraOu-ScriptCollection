...
`w` コマンドで作られる `book/backup/book_miner-....db` もやねうら王の通常定跡形式ですが、対局用には peta shock 化後の `peta_book-....db` または外部で作った `.ybb` を使うのが基本です。
...
やねうら王エンジンの実行ファイルがあるフォルダに `book/` フォルダを用意し、使いたい `peta_book-....db` または `.ybb` をコピーします。
...
- `BookFile` が使いたい `peta_book-....db` または `.ybb` になっているか。
- `BookDir` から見た場所に、その peta book があるか。
