...
`max_book_ply` に到達する局面は、次に掘る局面としては書き出されません。GUIでは各 peta 操作行の `game ply limit` 欄、CLIでは `pn` コマンドの第2引数で変更できます。
...
`pn` コマンドを実行しても、`book/backup/peta_book-....db` をファイルから読み直すわけではありません。
...
手順2. peta_next または peta next refu. または peta refutation または peta depth_gap または peta unsolved
手順3. enqueue
```
