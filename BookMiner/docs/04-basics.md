...
GUI の `enqueue進捗` は、BookMiner を起動してから enqueue した累計タスク数に対して、処理が完了したタスク数を表示します。

```text
enqueue進捗 30000/50000
```

これは「これまで enqueue した 50000 タスクのうち 30000 タスクが完了した」という意味です。

もう一度 enqueue すると、総数は追加分だけ増えます。例えば 50000 タスク中 30000 タスクが完了した状態で 72462 行を追加 enqueue した場合、進捗は `30000/122462` のようになります。
...
`makebook peta_shock` の意味、`peta_book` が必要な理由、`value` / `depth` の扱いは [10. peta shock 化](10-peta-shock.md) にまとめています。`peta next`、`peta refutation`、`peta depth gap`、`peta unsolved`、`peta opponent` の使い分けは [11. peta book を使って次に掘る局面を作る](11-peta-operations.md) にまとめています。
...
```text
p で peta_book を作って読む
pr 100 30 9999 200 None 400 で反駁leafを book/think_sfens.txt に書き出す
e で book/think_sfens.txt を探索 queue に積む
```
...
`game_ply_limit` に到達する局面は、次に掘る局面としては書き出されません。GUIでは各 peta 操作行の `game ply limit` 欄、CLIでは `pn` コマンドの第3引数で変更できます。
...
引数は `eval_refutation_margin eval_diff max_step game_ply_limit book_extend_ply eval_limit` の順です。peta shock 後の局面で、
...
例えば、ある局面以降だけを深く掘りたい場合、その局面を `book/peta_start_sfens.txt` に書いてから `peta next` を実行します。
...
手順2. peta next または peta refutation または peta depth gap または peta unsolved または peta opponent
手順3. enqueue
```

`peta_shock` は、いままで掘った通常定跡 DB を peta shock 化して、次に掘る局面を高速に探せる形にします。
`peta_shock_latest` は、現在DBを保存せず、`book/backup/` の最新通常bookを peta shock 化します。直前に自動保存された通常bookを使えば十分な場合に、二重保存を避けられます。
...
`peta depth gap` は、`peta next` と同じ範囲で、best に近い評価値だが depth が浅い候補手について、その候補手のPV leafを `book/think_sfens.txt` に書き出します。
