...
棋譜の末端まで到達できた場合は、そこからエンジンの best line を `think_ply` 手分だけ延長して掘ります。デフォルトは `6` で、CLIでは `t` コマンドの第3引数、GUIでは `enqueue` 行の `think ply` 欄で変更できます。この延長中も、評価値の絶対値が eval limit を超えたら停止します。
