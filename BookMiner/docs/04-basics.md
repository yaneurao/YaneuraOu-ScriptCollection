...
`max_book_ply` に到達する局面は、次に掘る局面としては書き出されません。GUIでは各 peta 操作行の `game ply limit` 欄、CLIでは `n` コマンドの第3引数で変更できます。
...
また、通常の `peta_next` で辿った leaf のうち、定跡から抜ける最後の1手が反駁された手だったものだけを出力する `nf` コマンドもあります。GUIでは `peta next refu.` がこれに対応します。
...
手順2. peta_next または peta next refu. または peta refutation または peta depth_gap
手順3. enqueue
```
