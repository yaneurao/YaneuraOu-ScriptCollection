...
反駁された leaf を重点的に延長したい場合は、`pn` の代わりに `pnf` を使います。

```text
p で peta_book を作って読む
pnf 30 100 9999 200 None で反駁leafを book/think_sfens.txt に書き出す
t で book/think_sfens.txt を探索 queue に積む
```
...
通常の `peta_next` で辿った leaf のうち、定跡から抜ける最後の1手が反駁された手だったものだけを出力する `pnf` コマンドがあります。GUIでは `peta next refu.` がこれに対応します。
...
GUI では、手順2の `peta next refu.` ボタンと `eval refu.` 入力欄がこの機能に対応します。抽出後は通常通り `enqueue` で `book/think_sfens.txt` を探索 queue に積みます。
...
手順2. peta_next または peta next refu. または peta next gap または peta unsolved または peta opponent
手順3. enqueue
```
...
`peta next gap` は、`peta_next` と同じ範囲で、best に近い評価値だが depth が浅い候補手について、その候補手のPV leafを `book/think_sfens.txt` に書き出します。
