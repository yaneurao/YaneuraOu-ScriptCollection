...
| Windows x86 | `Windows x86` | `C:\msys64\mingw64.exe` | `MINGW64` |

MSYS2はenvironmentごとにPATHとpackage prefixが分かれています。Windows x86版は、
`MINGW32` の `g++` ではなく、`MINGW64` の `clang++` に `--target=i686-w64-windows-gnu` を付けて作ります。
...
- Windows x86: `C:\msys64\mingw64.exe`
...
x86版もビルドする場合は、`MINGW64` shellで実行します。
...
```bash
echo $MSYSTEM
clang++ --target=i686-w64-windows-gnu --version
which make
which 7z
```

`echo $MSYSTEM` は `MINGW64` になっている必要があります。x86ビルドでは、現行の
YaneuraOu-Builder は `clang++` に `--target=i686-w64-windows-gnu` を自動で付けます。
...
`clang++` が見つからない場合や、`clang++ --target=i686-w64-windows-gnu --version` が失敗する場合は、必要なpackageを入れたか確認します。
`make[1]: *** [... .o] エラー 127` が compiler 実行直後に出る場合も、ほぼ同じ原因です。
...
x64ビルドもx86ビルドも `MINGW64` で実行します。
x86ビルドで target 指定が失敗する場合、または `pthread.h` が見つからない場合は、
`MINGW64` shellで次を入れてください。

```bash
pacman -S --needed mingw-w64-x86_64-clang mingw-w64-x86_64-lld mingw-w64-i686-gcc mingw-w64-i686-winpthreads
```

Windows x64 の link 時に `lld` が見つからない場合は、`MINGW64` 側で次を入れてください。
