...
Windows x64 / x86でビルドする場合は、MSYS2の `MINGW64` 環境を使います。
...
x86版は `MINGW32` の `g++` ではなく、`MINGW64` の `clang++` に `--target=i686-w64-windows-gnu` を付けて
ビルドします。`pthread.h` は `mingw-w64-i686-winpthreads` で入る `/mingw32/include`
を参照します。
...
Windows x64もWindows x86も、GUI実行時は `MINGW64` で実行します。
