...
`-index.ybb` には局面 index を置きます。
`-moves.ybb` には指し手列を置きます。

2ファイルに分けることで、やねうら王本体が on-the-fly probe するときに、index を二分探索して必要な指し手列だけを moves file から読むことができます。
...
通常の定跡ファイルとして使う場合は、`BookFile` に `-index.ybb` 側を指定します。

```text
setoption name BookFile value user_book-index.ybb
