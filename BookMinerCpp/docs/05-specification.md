...
book_miner-YYYYMMDDHHMMSS_N-index.ybb
```
...
book_miner-YYYYMMDDHHMMSS_N_ply100-index.ybb
*.tmp
```
...
```text
book_miner-....-index.ybb.tmp
book_miner-....-moves.ybb.tmp
```

両方を書き終えてから正式名へ rename します。
...
- 通常の `BookFile=user_book-index.ybb`
- `BookOnTheFly=true` での二分探索 probe
- `FlippedBook=true` 時の packed sfen 直接 flip probe
- `makebook peta_shock` の入力

`BookOnTheFly=true` の場合、index file を二分探索し、hit した局面の moves だけを moves file から読みます。
