...
- `--from_gui`
- 起動時の `book/backup/book_miner-*-index.ybb` 読み込み
- 互換用の `book/backup/book_miner-*.db` 読み込み
- やねうら王標準定跡フォーマットの読み込み
- やねうら王 バイナリ定跡DB (`.ybb`) の読み込み
- `w` / `q` による `book/backup/book_miner-YYYYMMDDHHMMSS_N-index.ybb` / `book_miner-YYYYMMDDHHMMSS_N-moves.ybb` 書き出し
- `*-index.ybb.tmp` / `*-moves.ybb.tmp` への一時書き出し後に正式名へ置換する安全な保存
- `r` による `YO-MATERIAL.exe` 呼び出し、peta shock 化、`peta_book-....db` の読み込み
...
- `.ybb` への書き出し時は sorted run 群を `PackedSfen` の32 byte順で n-way merge し、固定長indexと可変長movesを別ファイルに保存
...
`settings/book_miner_settings.json5` の `auto_save_interval_seconds` に従って、定期的に `book/backup/book_miner-YYYYMMDDHHMMSS_N-index.ybb` と `book/backup/book_miner-YYYYMMDDHHMMSS_N-moves.ybb` を書き出します。
...
`.ybb` 書き出しでは、run 群と一時的にsortした memtable run を n-way merge しながら index/moves を直接書き出します。
...
生成先は一度 `*.db.tmp` とし、成功後に `*.db` へ置換します。
正式な `YO-MATERIAL.exe` の配置場所は `BookMinerCpp/YO-MATERIAL.exe` です。
開発時のみ `../BookMiner/YO-MATERIAL.exe` もフォールバックとして探します。

やねうら王側にも `.ybb` reader を追加し、通常の `BookFile=user_book-index.ybb` と、`makebook peta_shock` の入力の両方で使えるようにします。
peta shock 後の出力は当面、従来のやねうら王標準定跡 `.db` のままです。
...
move file は `flags` によって 4 byte または 6 byte record の連続です。
