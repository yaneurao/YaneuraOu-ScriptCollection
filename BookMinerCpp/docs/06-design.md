...
- `pn` による peta next
- `pnf` による peta_next_refutation
- `pf` による peta_refutation
- `pd` による peta_depth_gap
- `pu` による peta_unsolved
- `po` による peta_opponent
...
`t [eval_limit] [max_book_ply] [think_command_ply]` は `book/think_sfens.txt` の `startpos moves ...` 形式の各行を `TaskQueue` に積み、起動済みUSIエンジンごとに1本のworker threadが処理します。
各行に `book_extend_ply=...` メタ情報がある場合、そのtaskだけ `think_command_ply` を上書きします。
...
`pn peta_eval_diff [max_book_ply] [max_step]` は、メモリに読み込まれている peta book を辿り、次に掘る局面を `book/think_sfens.txt` に書き出します。
...
`pnf peta_eval_diff [max_book_ply] [max_step] [eval_refutation_margin]` は、`peta next` のleafのうち、定跡から抜ける最後の1手が通常book上ではbestではなかったものだけを書き出します。判定はPython版と同じく、peta_book上の `反駁候補手評価値 - 旧best手評価値 >= eval_refutation_margin` です。

`pf [eval_refutation_margin] [eval_limit] [max_book_ply]` は、peta shock 後に best になった depth 0 の反駁候補を全node走査で書き出します。

`pd [eval_per_ply] [max_book_ply]` は、best に近いが depth が浅い候補手について、depth差ぶん延長すると逆転しうるPV leafを書き出します。
...
`po [eval_diff] [max_step] [game_ply_limit] [book_extend_ply] [eval_limit]` は、`book/book_opponent/` の相手定跡と現行 peta_book を仮想対局させ、どちらかの定跡が切れた地点から現行 peta_book のPV leafまで辿った局面を書き出します。C++版は相手定跡として `.db` と `.ybb` の両方を扱い、相手定跡は全読み込みせず on-the-fly probe します。`book_extend_ply`、`eval_limit`、`game_ply_limit` を指定した場合、出力行に対応するメタ情報を付けます。
