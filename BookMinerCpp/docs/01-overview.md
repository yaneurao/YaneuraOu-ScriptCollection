...
そのため、GUI のボタン、ログ欄、progress bar、`peta_shock`、`peta_shock_latest`、`peta_read`、`peta next`、`peta refutation`、`peta unsolved`、`peta opponent`、`enqueue` の考え方は Python 版と同じです。
...
- `pr` / `pu` / `po` は条件を変えて次に掘る局面を `book/think_sfens.txt` に書き出します。
