...
```bash
python3 convert_db_to_ybb.py input.db output-index.ybb
```
...
`output.ybb` のように指定した場合は、`output-index.ybb` / `output-moves.ybb` として出力します。
...
| オプション | 既定値 | 意味 |
| --- | ---: | --- |
| `--tmp-dir DIR` | `./tmp` | 一時ファイルを作るディレクトリ。 |
...
```bash
python3 convert_ybb_to_db.py input-index.ybb output.db
```

入力は `-index.ybb` 側を指定します。
対応する `-moves.ybb` は同じフォルダから自動的に読みます。
