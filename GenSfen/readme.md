...
```json5
{
    // 従来形式。kif/*.packに保存し、pack2hcpe.pyでHCPEへ変換する。
    "OUTPUT_FORMAT": "pack",
...
NNUE系で生成した教師データは、評価値がわりと上下にぶれるのでこれがノイズとなり得ると考えました。

そこで、pack2hcpe.pyで変換するときにN手先までの評価値を平滑化する機能を用意しました。

その時のNを`smoothing`(smoothing ply)として指定でき、割引率を`discount`(discount rate)として指定できます。

> python pack2hcpe.py kif20251110.pack --smoothing 3 --discount 0.9

このように指定した場合、現局面の評価値 , 1手先の評価値×0.9 , 2手先の評価値×0.9×0.9 の加重平均をとったものを現局面の評価値とします。つまり、

> 現局面の新しい評価値 = (現局面の評価値 + 1手先の評価値×0.9 + 2手先の評価値×0.9×0.9) / (1.0 + 0.9 + 0.9*0.9)
