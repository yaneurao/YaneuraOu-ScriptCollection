...
その対局ペアの両エンジンを再起動し、MultiPVの設定と`isready`/`readyok`の確認を行って、
開始局面集から次の局面を取り出して新しい対局を開始します。再接続は全対局スレッド間で直列化し、
各対局ペアにつき5秒・10秒・20秒の待機で最大3回試行します。`readyok`の直後に切断する場合も回数に含め、
教師局面を持つ対局が完了した場合に回数をリセットします。3回再接続しても対局が完了しなければ、その対局スレッドを停止します。
終了時は`stop`・`quit`を送信して最大5秒待ち、その後も終了しない場合にローカルプロセスを強制終了します。
SSHが既に切断されている場合や強制終了した場合は、リモートプロセスの終了は保証できません。
再起動が繰り返される場合は、gensfenを停止し、例外ログとリモート側のプロセス状態を確認してください。
