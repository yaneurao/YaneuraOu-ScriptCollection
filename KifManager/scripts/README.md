...
進捗は標準エラーに出力します。デフォルトでは1000ファイルごとに表示し、間隔を変える場合は `--progress-interval N`、進捗を止める場合は `--no-progress` を指定します。

戦績集計ではCSAの `N+`, `N-`, 指し手の先後、終局行だけを軽くスキャンします。合法手parseやUSI変換は行いません。
